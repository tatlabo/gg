...
def main():
        ...
        for k in lista_folderow_count:
            if any(substring in k[1].replace('\\', ' ') for substring in notthisfolder()):
                ...
                for m in filelist(k[1]):
                    ...
                    if extension.lstrip('.') in file_extensions():
                    #     continue
                    # else:
    ...
    files = cur.execute('SELECT path.name, file.name FROM file JOIN path ON file.path_id = path.id;').fetchall()

    for j in files:
        cur.execute('CREATE TABLE IF NOT EXISTS timecode (id INTEGER PRIMARY KEY UNIQUE, name TEXT, edl TEXT, start TEXT, end TEXT, vin TEXT, vout TEXT);')
        conn.commit()

        with open (f'{j[0]}\\{j[1]}') as fin:
            for line in fin:
                if re.search(r'^\d{3}?\s{2}?', line):
                    li = line.split()
                    cur.execute(f'INSERT INTO timecode (name, edl, start, end, vin, vout) VALUES ("{li[1]}", "{j[1]}", "{li[4]}", "{li[5]}", "{li[6]}", "{li[7]}");')
            conn.commit()
